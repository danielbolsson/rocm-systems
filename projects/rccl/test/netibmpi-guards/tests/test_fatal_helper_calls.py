...
def test_splits_parameters_containing_arrow(tmp_path):
    # `->` in a default argument used to drive the splitter's depth negative,
    # merging every parameter into one and losing the out-parameter.
    root = _write(tmp_path, "class F {\n"
                            "    void SetupThing(void* ctx, void** comm) {\n"
                            "        ASSERT_EQ(MakeThing(ctx->inner, comm), 0);\n    }\n};\n")
    names = {v.helper for v in fh.find_violations(root)}
    assert "SetupThing" in names
